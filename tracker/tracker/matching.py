...
def ious(atlbrs, btlbrs):
    ...
    """atlbrs = np.stack(atlbrs)
    btlbrs = np.stack(btlbrs)

    atlbrs = atlbrs[:, None, :]  # shape (3, 1, 4)
    btlbrs = btlbrs[None, :, :]  # shape (1, 1, 4)

    #print(atlbrs,'b',btlbrs)

    centerx1 = (atlbrs[..., 0] + atlbrs[..., 2]) / 2.0
    centery1 = (atlbrs[..., 1] + atlbrs[..., 3]) / 2.0
    centerx2 = (btlbrs[..., 0] + btlbrs[..., 2]) / 2.0
    centery2 = (btlbrs[..., 1] + btlbrs[..., 3]) / 2.0

    inner_diag = (centerx1 - centerx2) ** 2 + (centery1 - centery2) ** 2

    xxc1 = np.minimum(atlbrs[..., 0], btlbrs[..., 0])
    yyc1 = np.minimum(atlbrs[..., 1], btlbrs[..., 1])
    xxc2 = np.maximum(atlbrs[..., 2], btlbrs[..., 2])
    yyc2 = np.maximum(atlbrs[..., 3], btlbrs[..., 3])

    outer_diag = (xxc2 - xxc1) ** 2 + (yyc2 - yyc1) ** 2
    diou = ious - inner_diag / outer_diag
